export default {
    updateTitle:'Update title',
    prayer:'CONFESSION',
    todaysDevotional:'TODAYS DEVOTIONAL',
    furtherStudy:'FURTHER STUDY',
    oneyrBibleReadingPlan:'1 - YEAR BIBLE READING PLAN',
    secondyrBibleReadingPlan:'2 - YEAR BIBLE READING PLAN',
    API_URL:"https://backend3.rhapsodyofrealities.org",
    BOOKS_URL:"https://books-api.rhapsodyofrealities.org",
    VERSE_URL:"https://bibleapi.rhapsodyofrealities.org/",
    BRIDGE_URL:"https://api-bridge.rhapsodysubscriptions.org/",
    REGISTER_RHAPSODYPLUS_USER: "https://rhapsodysubscriptions.org/user/register",
    RESEND_CODE :"https://api-bridge.rhapsodysubscriptions.org/mailer/user/send_verification_code",
    SUBMIT_SUPPORT : "https://backend3.rhapsodyofrealities.org/insert_support",
    READING:'',
    TABLE_NAME:"book",
    TABLE_NAME_DOWNLOAD:"book_download",
    DATABASE_NAME:"MyBook.db"


}